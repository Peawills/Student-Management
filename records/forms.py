...
class StudentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["classroom"].queryset = ClassRoom.objects.order_by("level", "arm")
        self.fields["classroom"].label = "Class at Present"

        # Apply Tailwind CSS classes to form fields for consistent styling
        text_classes = "block w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-colors"
        select_classes = text_classes
        textarea_classes = text_classes + " h-28 resize-vertical"
        checkbox_classes = (
            "h-5 w-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
        )
        file_classes = "block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"

        for field_name, field in self.fields.items():
            widget = field.widget
            widget_class = widget.__class__.__name__

            if widget_class == "FileInput":
            ...
            else:  # TextInput, DateInput, NumberInput, etc.
...
class MultipleStudentDocumentForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            ...
            if widget_class in ["FileInput", "ClearableFileInput", "MultiFileInput"]:
                widget.attrs["class"] = file_classes
            elif widget_class == "Textarea":
                widget.attrs["class"] = textarea_classes
            elif widget_class == "Select":
                widget.attrs["class"] = select_classes
            else:  # TextInput, ChoiceField, etc.
                widget.attrs["class"] = text_classes
...
class StudentSearchForm(forms.Form):
    ...
    klass = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Filter by class..."}),
        label="",
    )
