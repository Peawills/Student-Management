...
def student_list(request):
    ...
    # Apply sorting
    allowed_sorts = ["-id", "id", "surname", "-surname", "admission_no", "-admission_no"]
...
def admin_dashboard(request):
    ...
    try:
        ...
        students_with_docs = Student.objects.filter(documents__isnull=False).distinct().count()
...
def student_detail(request, pk):
    ...
    try:
        student = get_object_or_404(
            Student.objects.prefetch_related("documents"), 
            pk=pk
        )
            ...
            try:
                with transaction.atomic():
                    ...
                    if multiple_docs_form.is_valid():
                        ...
                        doc_type = multiple_docs_form.cleaned_data.get("document_type", "other")
                        name_template = multiple_docs_form.cleaned_data.get("name") or "Document"
                        
                        for idx, f in enumerate(files, 1):
                            doc_name = f"{name_template} {idx}" if len(files) > 1 else name_template
...
def delete_document(request, student_id, doc_id):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(
                request,
                f"Document '{doc_name}' deleted successfully."
            )
            return redirect("student_detail", pk=student.id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.error(f"Error bulk uploading documents for student {student_id}: {str(e)}")
